...
4. “行会1成员””行会2成员“改成自己想设置的名称前缀
5. 运行本程序
'''
'''
注1： 默认是40_1 40_2的第一个账号开始编号为1，不会改会长名字，想自动改会长名字可以在俩TXT里先加上 跑完了再删掉，或者干脆手动改
注2： 只是单纯改名不用到加入行会的功能的话直接把runmain里的加入行会的部分，即a.joinhanghui(clubname)注释掉
注3： 如果是29对1的1个行会进行改名的话直接复制zhanghao.txt的到40_1.txt里或者程序里读取的账号文件名改成zhanghao.txt。然后注释掉40_2开始的部分（不注释应该也行）
'''
